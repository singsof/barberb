            ...
            <form id="form-edit_ser" action="javascript:void(0)" method="post">
                ...
                <div class="mb-3">
                    ...
                    <select name="id_hai" class="form-control" required>
                        ...
                        foreach (Database::query($sql_search, PDO::FETCH_OBJ) as $rowx) :
                        ?>
                            <option <?php echo $row->id_hai == $rowx->id_hai ? "selected" : "" ?>  value="<?php echo $rowx->id_hai ?>"><?php echo $rowx->name_hai ?></option>
                        <?php endforeach; ?>
                    </select>
                </div>
                <div class="form-group">
                    <button type="submit" class="btn btn-primary">แก้ไขข้อมูล</button> <button type="button" onclick="location.assign('index.php')" class="btn btn-warning">ยกเลิก</button>
                </div>
